...
# [중요] DB 이름을 v8로 변경합니다.
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'basket_v8.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)

# --- [1. 데이터 모델: 여기에 User가 있어야 에러가 안 납니다] ---
...
class Category(db.Model):
    ...
    type = db.Column(db.String(20)) # 'MAIN'(대분류) 또는 'SUB'(소분류)
    parent_id = db.Column(db.Integer, db.ForeignKey('category.id'), nullable=True)
    sub_categories = db.relationship('Category', backref=db.backref('parent', remote_side=[id]), lazy=True)
...
def index():
    ...
    return render_template('index.html', products=products, current_cat=cat, current_sub=sub, main_cats=main_cats)
